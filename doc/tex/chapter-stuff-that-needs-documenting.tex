...
some uniontype, consider renaming it at the point of definition, then 
...
A tutorial on cascading uniontypes might be helpful.  A relatively tractable 
...
type variables by a leading underscore: 
   ...
   _A _B _C ... _X _Y _Z               # Equality type variables.  
    ...
    A_descriptive_type_variable        # Long vanilla  type variable:  [A-Z]_[a-z0-9_']+ 
   _A_descriptive_type_variable        # Long equality type variable: _[A-Z]_[a-z0-9_']+ 
...
Base equality types are: 
        ...
        Void     ()   
        Bool     TRUE FALSE           # This is really a special case of uniontypes, treated below. 
...
(NB: Null_Or() and List() are both in fact special-case uniontype 
definitions  --- we list them separately in the interests of putting 
tutorial clarity before mathematical minimality.) 

The interesting corner cases arise from uniontype definitions. 

First, uniontype definitions need not reference any subtypes, 
...
Also, uniontype definitions can be recursive: 
...
Most generally, however, uniontype definitions may refer to 
...
As we saw earlier, a uniontype declaration is used to introduce a new 
...
an extension of the uniontype and recursive-val declaration forms.  We will 
...
HOW TO WRITE CODE THAT "JUST WORKS(TM)"
 ...
 o Use enumerated uniontypes instead of string or int constants,
   ...
   to a uniontype, the compiler will tell you about every place in the
   ...
   structured uniontypes with clearer semantics.
...
'  like weeds after a spring rain.
   ...
   Use enumerated uniontypes whenever reasonably possible.

   Even when you need integers, use type abstraction or uniontype wrappers
