...
then a uniontype with one constructor for each defaultable keyword 
...
The very special {\bf Exception} uniontype is also predefined.
...
Finally, Mythryl's uniontype definition facility effectively 
...
One major exception is that every uniontype declaration creates a 
new type:
...
\begin{itemize}
\item Every uniontype definition introduces a new type.
...
The predefined Mythryl uniontype {\tt Exception} is unique in that it may 
be incrementally extended by defining new constructors for it using 
{\tt exception} declarations.

Vanilla Mythryl uniontype declarations require that all constructors belonging 
to the type be declared up front at the point of uniontype definition;  no later 
extension of the uniontype is allowed.  Normally this is good;  it means that 
when you read a uniontype definition in the code you can be sure that what 
you see is the complete story.

But situations can occasionally arise in industrial-scale Mythryl programming in 
which it is desirable to incrementally extend a uniontype.

Programmers can and do simply use the {\tt Exception} uniontype directly in 
...
Here is a hack to define your own extensible uniontypes separate from 
the standard Mythryl {\tt Exception} uniontype:
...
type parameter;  we added the phantom type without changing the runtime uniontype 
in any way.
...
uniontypes to use as the phantom types, just to improve readability:
