...
my(@sml_keywords) = 
   qw(functor structure signature sig struct sharing where and
      let open include fun val rec end case raise handle withtype
      datatype type infix infixr nonfix fn local in as
...
sub smltype
{  my($type) = @_;
   return '<span class="smltype">' . $type . "</span>";
}

sub smlstring
{  my($s) = @_;
   return '<span class="smlstring">' . $s . "</span>";
}


sub smlnum
{  my($type) = @_;
   return '<span class="smlnum">' . $type . "</span>";
}

sub smlconst 
{  my($const) = @_;
   return '<span class="smlconst">' . $const . "</span>";
}
