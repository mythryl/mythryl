...
package mythryl_token_table
    =
    mythryl_token_table_g( tokens );	# Defined in ROOT/src/lib/compiler/frontend/parse/lex/mythryl-token-table-g.pkg
...
fun eof ( { comment_nesting_depth, err, stringlist, stringstart, source_map, ... } : Lex_Arg)
    =
...
%%
<initial>{ws}	=> (continue());
<initial>{eol}	=> (source_map::newline source_map yypos; continue());
...
<initial>"&"{eol}	=> (source_map::newline source_map yypos; tokens::amper(yypos,yypos+1));
<initial>"@"{eol}	=> (source_map::newline source_map yypos; tokens::atsign(yypos,yypos+1));
<initial>"\\"{eol}	=> (source_map::newline source_map yypos; tokens::back(yypos,yypos+1));
<initial>"!"{eol}	=> (source_map::newline source_map yypos; (tokens::uppercase_id (fast_symbol::raw_symbol ((hash_string "!"), "!"), yypos, yypos+1)));
<initial>"|"{eol}	=> (source_map::newline source_map yypos; tokens::bar(yypos,yypos+1));
<initial>"$"{eol}	=> (source_map::newline source_map yypos; tokens::buck(yypos,yypos+1));
<initial>"^"{eol}	=> (source_map::newline source_map yypos; tokens::caret(yypos,yypos+1));
<initial>"-"{eol}	=> (source_map::newline source_map yypos; tokens::dash(yypos,yypos+1));
<initial>"{"{eol}	=> (source_map::newline source_map yypos; tokens::lbrace(yypos,yypos+1));
<initial>"<"{eol}	=> (source_map::newline source_map yypos; tokens::langle(yypos,yypos+1));
<initial>">"{eol}	=> (source_map::newline source_map yypos; tokens::rangle(yypos,yypos+1));
<initial>"%"{eol}	=> (source_map::newline source_map yypos; tokens::percnt(yypos,yypos+1));
<initial>"+"{eol}	=> (source_map::newline source_map yypos; tokens::plus(yypos,yypos+1));
<initial>"?"{eol}	=> (source_map::newline source_map yypos; tokens::qmark(yypos,yypos+1));
<initial>"/"{eol}	=> (source_map::newline source_map yypos; tokens::slash(yypos,yypos+1));
<initial>"*"{eol}	=> (source_map::newline source_map yypos; tokens::star(yypos,yypos+1));
<initial>"~"{eol}	=> (source_map::newline source_map yypos; tokens::tilda(yypos,yypos+1));
<initial>"."{eol}	=> (source_map::newline source_map yypos; (tokens::operators_id (fast_symbol::raw_symbol ((hash_string " . "), " . "), yypos, yypos+1)));
<initial>"++"{eol}	=> (source_map::newline source_map yypos; tokens::plus_plus(yypos,yypos+2));
<initial>"--"{eol}	=> (source_map::newline source_map yypos; tokens::dash_dash(yypos,yypos+2));
<initial>".."{eol}	=> (source_map::newline source_map yypos; tokens::dotdot(yypos,yypos+2));
...
<postfix>{eol}	=> (source_map::newline source_map yypos; yybegin initial; continue());
...
<llc>"*/"                 => (yybegin initial; my_synch(source_map, yypos+2, *stringlist); 
		              comment_nesting_depth := 0; stringlist := []; continue());
<llc>{ws}\"		  => (yybegin llcq; continue());
<llcq>[^\"]*              => (add_string(stringlist, yytext); continue());
<llcq>\""*/"              => (yybegin initial; my_synch(source_map, yypos+3, *stringlist); 
		              comment_nesting_depth := 0; stringlist := []; continue());
...
<comment>{eol}	=> (source_map::newline source_map yypos; yybegin initial; continue());
<comment>.		=> (continue());

<aaa>"/*"[*=#-]*	=> (inc comment_nesting_depth; continue());
<aaa>{eol}	=> (source_map::newline source_map yypos; continue());
...
<string>\\{eol}	=> (source_map::newline source_map (yypos+1);
...
<char>{eol}	=> (err (*stringstart,yypos) ERROR "unclosed string"
		    ...
		    source_map::newline source_map yypos;
		    yybegin initial; tokens::string(make_string stringlist,*stringstart,yypos));
<char>\\{eol}     	=> (source_map::newline source_map (yypos+1);
		    yybegin stringgap; continue());
...
<stringgap>{eol}	=> (source_map::newline source_map yypos; continue());
...
<qqq>{eol}        => (source_map::newline source_map yypos; add_string(stringlist,"\n"); continue());
<qqq>.            => (add_string(stringlist,yytext); continue());

<aq>{eol}       => (source_map::newline source_map yypos; continue());
<aq>{ws}        => (continue());
<aq>{id}        => (yybegin qqq; 
