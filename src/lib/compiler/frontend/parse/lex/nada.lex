...
package mada_token_table
    =
    mada_token_table_g( tokens );		# Defined in ROOT/src/lib/compiler/frontend/parse/lex/nada-token-table-g.pkg
...
fun eof ( { comment_nesting_depth, err, charlist, stringstart, source_map, ... } : Lex_Arg)
    =
    {   pos =
 ...
 *    the like, but those are all handled in re-lex.pkg.
 *
...
<initial>{eol}	=> (source_map::newline source_map yypos;
                    tokens::raw_whitespace(yypos,yypos+size yytext));
...
<llcq>\""*/"              => (yybegin initial; my_synch(source_map, yypos+3, *charlist); 
...
<comment>{eol}		=> (source_map::newline source_map yypos; continue());
...
<indent>{eol}		=> (source_map::newline source_map yypos; continue());
...
<quote>{eol}    => (source_map::newline source_map yypos; add_string(charlist,"\n"); continue());
...
<antiquote>{eol} 	=> (source_map::newline source_map yypos; continue());
