...
various modules in the LOWCODE customizable code generator
...
\begin{enumerate}
    ...
    \item How instructions are encoded in machine code: LOWCODE uses
...
for debugging and also for assembly output for other backends.
    \item How instructions are internally represented in LOWCODE. 
...
LOWCODE manipulates all instruction sets via a set of abstract
...
an MDGen specification elaborates.  
   But generally speaking, a machine description is a just a package 
...
purpose registers and 32 floating point registers.  In addition, LOWCODE
requires that each architecture specifies a (pseudo) register 
type\footnote{Called cellkind in LOWCODE.} for 
...
real register type. 
  \item Cellsets
are used by LOWCODE for annotating liveness information in the program.
...
always has a fixed value.  
  ...
  By default, a field has type \sml{unt32.word}.  If a type $T$ 
...
\subsubsection{Generating Encoding Functions}
   ...
   In LOWCODE, we represent an instruction as a set of ML datatypes.
...
\subsection{Specifying Instruction Semantics}
   LOWCODE performs all optimizations at 
...
\begin{itemize}
  ...
  \item Our RTL language, like that of MDGen, is tied closely to LOWCODE.
\end{itemize}
...
\begin{itemize}
 \item \lowcodehref{sparc/sparc.mdl}{Sparc} 
 \item \lowcodehref{ppc/ppc.mdl}{PowerPC} 
 \item \lowcodehref{x86/x86.mdl}{X86} 
\end{itemize}
