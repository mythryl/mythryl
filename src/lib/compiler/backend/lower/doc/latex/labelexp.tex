...
\mlrischref{instruction/labelExp.sml}{LabelExp}.

The enum \sml{labexp} has the following definition:
\begin{SML}
  enum labexp = 
...
a label expression
  \item \sml{to_string : labexp -> String} -- Return the pretty printed representation of an expression
...
\begin{SML}
   generic package \mlrischref{instruction/labelExp.sml}{LabelExp}(Constant : \mlrischref{instruction/constant.api}{CONSTANT})
\end{SML}
