\section{The MLRISC Architecture Description Language}

\subsection{ Overview }

\newdef{MDGen} is an architecture description language 
...
\begin{itemize}
  \item \codehref{src/lib/compiler/back/low/code/registerkinds.api}{Registerkinds} -- 
   the properties of the register set and (some part of) memory hierarchy. 
  \item \codehref{code/machcode-form.api}{Machcode\_Form} -- 
   the concrete instruction set representation.
  \item \codehref{code/instructionProps.sig}{INSTRUCTIONS_PROPERTIES}  --
   properties of the instructions.
  \item \codehref{code/compile-register-moves.api}{Compile_Register_Moves} --
   methods to emit linearized code from parallel copies.
  \item \codehref{emit/machcode-codebuffer.api}{ASSEMBLER} --
   the assembler
  \item \codehref{emit/machcode-codebuffer.api}{MC} --
   the machine code emitter
  \item \codehref{../jmp/jump-size-ranges.api}{ Jump_Size_Ranges } --
...
registerkinds \sml{GP}, \sml{FP}, \sml{CC} to be defined.
...
\begin{itemize}
  \item The registerkinds \sml{GP, FP, CC} are defined.
        Furthermore, the registerkinds \sml{MEM, CTRL}, which stand
        for memory and control (dependence), are also implicitly defined.
  \item The \sml{assembly as} clauses specify how a specific register type is
       to be displayed.    Here, we specify that register 30, the
       stack pointer, should be displayed specially as \sml{$sp}.
  \item The \sml{in registerset} clause, when attached, tells MDGen that
       the associated registerkind should be part of the 
       \href{registerset.html}{ registerset }.  The clause \sml{in registerset GP}
       tells MDGen that the a register of type \sml{CC} should be treated
...
\begin{SML}
   <font color=#FF0000>storage</font>
     GP = 32 <font color=#FF0000>registers <font color=#FF0000>of</font> 64 bits in registerset called</font> "register" 
       	<font color=#FF0000>assembly as</font> (\\ 30 => "$sp"
                      | r => "$"^int.to_string r)
   | FP = 32 <font color=#FF0000>registers <font color=#FF0000>of</font> 64 bits in registerset called</font> "floating point register" 
       	<font color=#FF0000>assembly as</font> (\\ f => "f"^int.to_string f)
   | CC = <font color=#FF0000>registers <font color=#FF0000>of</font> 64 bits in registerset GP called</font> "condition code register"
...
\begin{SML}
   ...
   <font color=#FF0000>enum</font> operand = 
     ...
     | HILABop <font color=#FF0000>of</font> LabelExp.label_expression       ``hi(<put_label_expression label_expression>)''
     | LOLABop <font color=#FF0000>of</font> LabelExp.label_expression       ``lo(<put_label_expression label_expression>)''
     | LABop <font color=#FF0000>of</font> LabelExp.label_expression       	``<put_label_expression label_expression>''
     | CONSTop <font color=#FF0000>of</font> Constant.const       ``<put_const const>''

   /* 
    * When I say ! after the enum</font> name XXX, it means generate a
    * function put_XXX that converts the constructors into the corresponding
...
\begin{SML}
   ...
   | LOAD <font color=#FF0000>of</font> {ldOp:load, r: <font color=#008800>$</font>GP, b: <font color=#008800>$</font>GP, d:operand, mem: region::region}
     ``<ldOp>\t<r>, <d>()''
     ILoadStore{opc=put_load ldOp,r,b,d}
   ...
   | STORE <font color=#FF0000>of</font> {stOp:store, r: <font color=#008800>$</font>GP, b: <font color=#008800>$</font>GP, d:operand, mem: region::region}
     ``<stOp>\t<r>, <d>()''
     ILoadStore{opc=put_store stOp,r,b,d}
   ...
   | FLOAD <font color=#FF0000>of</font> {ldOp:fload, r: <font color=#008800>$</font>FP, b: <font color=#008800>$</font>GP, d:operand, mem: region::region}
     ``<ldOp>\t<r>, <d>()''
     FLoadStore{opc=put_fload ldOp,r,b,d}
   ...
   | FSTORE <font color=#FF0000>of</font> {stOp:fstore, r: <font color=#008800>$</font>FP, b: <font color=#008800>$</font>GP, d:operand, mem: region::region}
     ``<stOp>\t<r>, <d>()''
     FLoadStore{opc=put_fstore stOp,r,b,d}
   ...
   | JSR <font color=#FF0000>of</font> {r: <font color=#008800>$</font>GP, b: <font color=#008800>$</font>GP, d:int} * C.registerset * C.registerset
     ``jsr\t<r>, <d>()''
   ...
   #  Integer Operate 
   | OPERATE <font color=#FF0000>of</font> {op:operate, ra: <font color=#008800>$</font>GP, rb:operand, rc: <font color=#008800>$</font>GP}
       ``<op>\t<ra>, <rb>, <rc>''
        (let my (opc,fn) = put_operate op
         in  Operate{opc,fn,ra,rb,rc} 
         end)

   | OPERATEV <font color=#FF0000>of</font> {op:operateV, ra: <font color=#008800>$</font>GP, rb:operand, rc: <font color=#008800>$</font>GP}
       ``<op>\t<ra>, <rb>, <rc>''
        (let my (opc,fn) = put_operateV op
         in  Operate{opc,fn,ra,rb,rc} 
         end)

   | PSEUDOARITH <font color=#FF0000>of</font> {op: pseudo_op, ra: <font color=#008800>$</font>GP, rb:operand, rc: <font color=#008800>$</font>GP, 
       	     tmps: C.registerset}
       ``<op>\t<ra>, <rb>, <rc>''
   ...
   | FCOPY <font color=#FF0000>of</font> {dst: <font color=#008800>$</font>FP list, src: <font color=#008800>$</font>FP list, 
               impl:instruction list option ref, tmp: ea option}
       ``<apply emitInstr (shuffle::compile_float_register_moves{regmap,tmp,dst,src})>''
 
   #  Floating Point Operate 
   | FOPERATE <font color=#FF0000>of</font> {op:foperate, fa: <font color=#008800>$</font>FP, fb: <font color=#008800>$</font>FP, fc: <font color=#008800>$</font>FP}
       ``<op>\t<fa>, <fb>, <fc>''
       (let my (opc,fn) = put_foperate op
        in  Foperate{opc,fn,fa,fb,fc}
        end)

   #  Trapping versions <font color=#FF0000>of</font> the above 
   | FOPERATEV <font color=#FF0000>of</font> {op:foperateV, fa: <font color=#008800>$</font>FP, fb: <font color=#008800>$</font>FP, fc: <font color=#008800>$</font>FP}
       ``<op>\t<fa>, <fb>, <fc>''
        Foperate{opc=0wx16,fn=put_foperateV op,fa,fb,fc}
 
   #  Misc 
   | TRAPB       			#  Trap barrier 
       ``trapb''
        Memory_fun{opc=0wx18,ra=31,rb=31,fn=0wx0}
   ...
   | CALL_PAL <font color=#FF0000>of</font> {code:osf_user_palcode, def: <font color=#008800>$</font>GP list, use: <font color=#008800>$</font>GP list}
       ``call_pal <code>''
        Pal{fn=put_osf_user_palcode code}
 end
\end{SML}
</tt>


\subsection{ 4 Architecture Descriptions }
Here are some architecture descriptions in varing degree of completion.

\begin{itemize}
 \item \codehref{../sparc32/sparc32.architecture-description}{ Sparc } 
 \item \codehref{../pwrpc32/pwrpc32.architecture-description}{ PowerPC } 
 \item \codehref{../intel32/one_word_int.architecture-description}{ Intel32 } 
\end{itemize}
