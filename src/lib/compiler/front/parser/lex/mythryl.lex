...
<initial>"{."	=> (tokens::lbrace_dot(yypos,yypos+2));
...
<string>\\\^.	=>
	(err(yypos,yypos+2) ERROR "illegal control escape; must be one of \
	 ...
	 continue());
<string>\\[0-7]{3}	=>
 ( {  x = (string::get_byte(yytext,1)-(char::to_int '0'))*64
	+ (string::get_byte(yytext,2)-(char::to_int '0'))*8
	+ (string::get_byte(yytext,3)-(char::to_int '0'));
      if   (x > 255)
...
<string>\\x[0-9][0-9]	=>
 ( {  x = ((string::get_byte(yytext,2) - char::to_int '0')     )*16
        + ((string::get_byte(yytext,3) - char::to_int '0')     );
...
<string>\\x[0-9][a-f]	=>
 ( {  x = ((string::get_byte(yytext,2) - char::to_int '0')     )*16
        + ((string::get_byte(yytext,3) - char::to_int 'a') + 10);
      add_char(stringlist, char::from_int x);
      continue();
  });
<string>\\x[0-9][A-F]	=>
...
<string>\\x[a-f][0-9]	=>
 ( {  x = ((string::get_byte(yytext,2) - char::to_int 'a') + 10)*16
        + ((string::get_byte(yytext,3) - char::to_int '0')     );
      add_char(stringlist, char::from_int x);
      continue();
  });
