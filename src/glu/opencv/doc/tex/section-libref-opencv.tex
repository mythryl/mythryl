...
Providing a production-quality Opencv binding in Mythryl cannot 
be done simply by providing one-to-one bindings to the Opencv 
C functions: 

\begin{itemize}

\item Mythryl has better namespace management than C, and consequently 
      has no need to add a clumsy {\tt opencv\_} prefix to every identifier.
...
\item To provide Mythryl levels of type safety, enum datatypes 
      need to be defined in place of Opencv integer constants like 
...
The Mythryl Opencv binding provides two interfaces intended for use by 
the application programmer:

\begin{itemize}
\item {\tt Easy\_Opencv}, which makes common simple GUI interfaces particularly easy to construct. 
\item {\tt Opencv\_Client}, which provides more complete and flexible access to the underlying Opencv library functionality. 
\end{itemize}

The {\tt Easy\_Opencv} interface is defined by  
\ahrefloc{src/glu/opencv/src/easy-opencv.api}{src/glu/opencv/src/easy-opencv.api}; the {\tt Opencv\_Client} interface is defined by 
\ahrefloc{src/glu/opencv/src/opencv-client.api}{src/glu/opencv/src/opencv-client.api}.

The Opencv interface uses tuple or record arguments to a function 
...
The Mythryl Opencv binding architecture is based upon a four-layer stack: 
\begin{itemize}
\item {\tt easy\_opencv}: High-level Mythryl application programmer functionality coded in Mythryl. 
\item {\tt opencv}: Low-level Mythryl application programmer functionality coded in Mythryl. 
\item {\tt mythryl-opencv-library-in-main-process}: Internal even lower-level functionality coded in Mythryl. 
\item {\tt mythryl-opencv-library-in-c-subprocess.c}: Lowest level, written in C and calling the actual Opencv library routines. 
\end{itemize}
...
The {\tt make-opencv-glue} script is normally invoked as needed by the 
...
\begin{verbatim}
    ...
    opencv-code : gtk_table_set_col_spacing( GTK_TABLE(/*table*/w0), /*col*/i1, /*spacing*/i2) 
...
\item {\bf url} if present points to the Opencv reference documentation on the relevant function. 
\item {\bf run} specifies the {\tt make-opencv-glue} function to be called to synthesize binding code. 
                The other fields are in essence arguments to this function. 
\item {\bf opencv-code} gives the actual C call to be made to the Opencv library. 
                ...
                A given parameter may appear more than once in the opencv code: 
                \begin{verbatim} 
...
\begin{verbatim}
    ...
    opencv-code : gtk_widget_set_size_request( GTK_WIDGET(/*widget*/w0), /*wide*/i1, /*high*/i2) 
    type     : (Session, Widget, (Int,Int)) -> Void 
    run      : plain-fn 
    url      : http://library.gnome.org/devel/gtk/stable/GtkWidget.html#gtk-widget-set-size-request 
    opencv-client-g.pkg:        fun set_minimum_widget_size (session: Session, widget, (wide, high)) 
    opencv-client-g.pkg:            = 
    opencv-client-g.pkg:            d::set_minimum_widget_size (session.subsession, widget, wide, high); 
\end{verbatim}
