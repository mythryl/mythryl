...
Providing a production-quality Ncurses binding in Mythryl cannot 
be done simply by providing one-to-one bindings to the Ncurses 
C functions: 

\begin{itemize}

\item Mythryl has better namespace management than C, and consequently 
      has no need to add a clumsy {\tt ncurses\_} prefix to every identifier.
...
\item To provide Mythryl levels of type safety, enum datatypes 
      need to be defined in place of Ncurses integer constants like 
...
The Mythryl Ncurses binding provides two interfaces intended for use by 
the application programmer:

\begin{itemize}
\item {\tt Easy\_Ncurses}, which makes common simple GUI interfaces particularly easy to construct. 
\item {\tt Ncurses\_Client}, which provides more complete and flexible access to the underlying Ncurses library functionality. 
\end{itemize}

The {\tt Easy\_Ncurses} interface is defined by  
\ahrefloc{src/glu/ncurses/src/easy-ncurses.api}{src/glu/ncurses/src/easy-ncurses.api}; the {\tt Ncurses\_Client} interface is defined by 
\ahrefloc{src/glu/ncurses/src/ncurses-client.api}{src/glu/ncurses/src/ncurses-client.api}.

The Ncurses interface uses tuple or record arguments to a function 
...
The Mythryl Ncurses binding architecture is based upon a four-layer stack: 
\begin{itemize}
\item {\tt easy\_ncurses}: High-level Mythryl application programmer functionality coded in Mythryl. 
\item {\tt ncurses}: Low-level Mythryl application programmer functionality coded in Mythryl. 
\item {\tt mythryl-ncurses-library-in-main-process}: Internal even lower-level functionality coded in Mythryl. 
\item {\tt mythryl-ncurses-library-in-c-subprocess.c}: Lowest level, written in C and calling the actual Ncurses library routines. 
\end{itemize}
...
The {\tt make-ncurses-glue} script is normally invoked as needed by the 
...
\begin{verbatim}
    ...
    ncurses-code : gtk_table_set_col_spacing( GTK_TABLE(/*table*/w0), /*col*/i1, /*spacing*/i2) 
...
\item {\bf url} if present points to the Ncurses reference documentation on the relevant function. 
\item {\bf run} specifies the {\tt make-ncurses-glue} function to be called to synthesize binding code. 
                The other fields are in essence arguments to this function. 
\item {\bf ncurses-code} gives the actual C call to be made to the Ncurses library. 
                ...
                A given parameter may appear more than once in the ncurses code: 
                \begin{verbatim} 
...
\begin{verbatim}
    ...
    ncurses-code : gtk_widget_set_size_request( GTK_WIDGET(/*widget*/w0), /*wide*/i1, /*high*/i2) 
    type     : (Session, Widget, (Int,Int)) -> Void 
    run      : plain-fn 
    url      : http://library.gnome.org/devel/gtk/stable/GtkWidget.html#gtk-widget-set-size-request 
    ncurses-client-g.pkg:        fun set_minimum_widget_size (session: Session, widget, (wide, high)) 
    ncurses-client-g.pkg:            = 
    ncurses-client-g.pkg:            d::set_minimum_widget_size (session.subsession, widget, wide, high); 
\end{verbatim}
