...
static Val   mkStatRep   (Task* task,  struct stat* buf)   {
    ...
    //
    set_slot_in_nascent_heapchunk(task,  0, MAKE_TAGWORD(12, PAIRS_AND_RECORDS_BTAG));
    ...
    set_slot_in_nascent_heapchunk(task,  8, high_32_bits_of_size);
    set_slot_in_nascent_heapchunk(task,  9,  low_32_bits_of_size);
    set_slot_in_nascent_heapchunk(task, 10, atime);
    set_slot_in_nascent_heapchunk(task, 11, mtime);
    set_slot_in_nascent_heapchunk(task, 12, ctime);
    //
    return commit_nascent_heapchunk(task, 12);
}
...
Val   _lib7_P_FileSys_stat_64   (Task* task,  Val arg)   {		//  : String -> statrep
	...
	RELEASE_MYTHRYL_HEAP( task->hostthread, "_lib7_P_FileSys_stat_64", NULL );
	    //
	    status = stat(c_path, &buf);
	    //
	RECOVER_MYTHRYL_HEAP( task->hostthread, "_lib7_P_FileSys_stat_64" );
...
Val   _lib7_P_FileSys_fstat_64   (Task* task,  Val arg)   {	//  : Unt -> statrep
    ...
    RELEASE_MYTHRYL_HEAP( task->hostthread, "_lib7_P_FileSys_fstat_64", NULL );
	//
	status = fstat(fd, &buf);
	//
    RECOVER_MYTHRYL_HEAP( task->hostthread, "_lib7_P_FileSys_fstat_64" );
...
Val   _lib7_P_FileSys_lstat_64   (Task* task,  Val arg)   {	//  : String -> statrep
	...
	RELEASE_MYTHRYL_HEAP( task->hostthread, "_lib7_P_FileSys_lstat_64", NULL );
	//
	status =  lstat( c_path, &buf );
	//
	RECOVER_MYTHRYL_HEAP( task->hostthread, "_lib7_P_FileSys_lstat_64" );
