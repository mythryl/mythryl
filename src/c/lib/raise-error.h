...
#include "system-dependent-stuff.h"
					// raise_sys_error__may_heapclean	def in     src/c/lib/raise-RUNTIME_EXCEPTION.c
...
Val raise_sys_error__may_heapclean (Task *task, int err, const char *alt_msg, const char *at, Roots* extra_roots);			// raise_sys_error__may_heapclean	def in    src/c/lib/raise-RUNTIME_EXCEPTION.c

#define RAISE_SYSERR__MAY_HEAPCLEAN(task, status, extra_roots)		raise_sys_error__may_heapclean((task), (status), NULL,  "<" __FILE__ ":" TOSTRING(__LINE__) ">", extra_roots)
#define RAISE_ERROR__MAY_HEAPCLEAN(task, msg, extra_roots)		raise_sys_error__may_heapclean((task), 0,        (msg), "<" __FILE__ ":" TOSTRING(__LINE__) ">", extra_roots)

#else

Val raise_sys_error__may_heapclean (Task *task, const char *alt_msg, const char *at, Roots* extra_roots);

#define RAISE_SYSERR__MAY_HEAPCLEAN(task, status, extra_roots)		raise_sys_error__may_heapclean((task), NULL,  "<" __FILE__  ":" TOSTRING(__LINE__) ">", extra_roots)
#define RAISE_ERROR__MAY_HEAPCLEAN(task, msg, extra_roots)		raise_sys_error__may_heapclean((task), (msg), "<" __FILE__  ":" TOSTRING(__LINE__) ">", extra_roots)
