...
//
void   do_dump_hugechunks_summary   (FILE* fd, Task* task, char* caller) {
    // ==========================
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Hugechunk datastructures dump generated by do_dump_hugechunks_summary in src/c/heapcleaner/heap-debug-stuff.c\n"			);
...
void   dump_hugechunks_summary   (Task* task, char* caller) {
    ...
    do_dump_hugechunks_summary   (fd, task, caller);
...
// (In practice this means all the compiledfiles in ram.)
//
void   do_dump_hugechunks_contents  (FILE* fd, Task* task, char* caller) {
    // ===========================
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Hugechunk datastructures contents dump generated by do_dump_hugechunks_contents in src/c/heapcleaner/heap-debug-stuff.c\n"		);
    fprintf(fd,"Most of this stuff is defined in one of:\n"												);
    fprintf(fd,"\n"																	);
    fprintf(fd,"    src/c/h/runtime-base.h\n"														);
    fprintf(fd,"    src/c/h/heap.h\n"															);
...
void   dump_all   (Task* task, char* caller) {
    ...
    do_dump_ramlog		(fd, task, caller);
    do_dump_hugechunks_summary	(fd, task, caller);
    do_dump_hugechunks_contents	(fd, task, caller);
