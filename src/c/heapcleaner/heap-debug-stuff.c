...
void   dump_hugechunk_stuff   (Task* task, char* caller) {
    ...
    fprintf(fd,"Hugechunk datastructures dump generated by dump_hugechunk_stuff in src/c/heapcleaner/heap-debug-stuff.c\n"				);
...
void   dump_ramlog   (Task* task, char* caller) {
    ...
    fprintf(fd,"Ramlog dump generated by dump_ramlog() in src/c/heapcleaner/heap-debug-stuff.c\n");

    fprintf(fd,"\nLast %d calls made from Mythryl to C, in time order (with task->pthread->id):\n\n");
