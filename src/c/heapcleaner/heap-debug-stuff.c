...
void   dump_gen0   (Task* task, char* caller) {
    ...
    fprintf(fd,"Generation-0 dump generated by dump_gen1 in src/c/heapcleaner/heap-debug-stuff.c\n"	);
    fprintf(fd,"\n"											);
...
void   dump_gen1   (Task* task, char* caller) {
    ...
    Heap*     heap =  task->heap;
    Agegroup* ag1  =  heap->agegroup[0];		// Get pointer to agegroup1.

    fprintf(fd,"Generation-1 dump generated by dump_gen1 in src/c/heapcleaner/heap-debug-stuff.c\n"	);
    fprintf(fd,"\n"											);
    fprintf(fd,"                     task p= %p\n",					task		);
    fprintf(fd,"                     heap p= %p  (task->heap)\n",			heap		);
    fprintf(fd,"                      ag1 p= %p  (agegroup1)\n",				ag1		);
    fprintf(fd,"\n"											);
    fprintf(fd,"                 ag1->age d= %d\n",					ag1->age	);
    fprintf(fd,"                 ag1->cleanings d= %d\n",				ag1->cleanings	);
    fprintf(fd,"                 ag1->ratio     d= %d\n",				ag1->ratio	);
    fprintf(fd,"                 ag1->last_cleaning_count_of_younger_agegroup d= %d\n",	ag1->last_cleaning_count_of_younger_agegroup	);
    fprintf(fd,"                 ag1->tospace_ram_region p= %p\n",			ag1->tospace_ram_region				);
    fprintf(fd,"                 ag1->fromspace_ram_region p= %p\n",			ag1->fromspace_ram_region			);
    fprintf(fd,"                 ag1->saved_fromspace_ram_region p= %p\n",		ag1->saved_fromspace_ram_region			);
    fprintf(fd,"                 ag1->coarse_inter_agegroup_pointers_map p= %p\n",	ag1->coarse_inter_agegroup_pointers_map		);
