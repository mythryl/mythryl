...
//
static void   dump_task_guts   (FILE* fd, Task* task, char* caller) {
    //        ==============
    //
    fprintf(fd,"Task dump generated by dump_task_guts in src/c/heapcleaner/heap-debug-stuff.c\n"	);
    fprintf(fd,"\n"										);
...
//
static void   dump_gen0_tripwire_buffers_guts   (FILE* fd, Task* task, char* caller) {
    //        ===============================
    //
...
void   dump_gen0_tripwire_buffers   (Task* task, char* caller) {
    ...
    dump_gen0_tripwire_buffers_guts   (fd, task, caller);
...
//
static void   dump_gen0_guts   (FILE* fd, Task* task, char* caller) {
    //        ==============
    //
    fprintf(fd,"Generation-0 dump generated by dump_gen0_guts in src/c/heapcleaner/heap-debug-stuff.c\n"	);
    fprintf(fd,"\n"											);
...
void   dump_gen0   (Task* task, char* caller) {
    ...
    dump_gen0_guts( fd, task, caller );

    close_heapdump_logfile( fd, filename );
    log_if("dump_gen0: Dump to '%s' now complete.", filename);
}
//
static void   dump_gen0s_guts   (FILE* fd, Task* task, char* caller) {
    //        ===============
    //
...
void   dump_gen0s   (Task* task, char* caller) {
    ...
    dump_gen0s_guts( fd, task, caller );
...
// to distinguish live from dead data.
//
static void   dump_gens_guts   (FILE* fd, Task* task, char* caller) {
    //        ==============
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Generation-1 dump generated by dump_gens_guts in src/c/heapcleaner/heap-debug-stuff.c\n"							);
    fprintf(fd,"\n"																	);
...
//
static void   dump_hugechunks_summary_guts   (FILE* fd, Task* task, char* caller) {
    //        ============================
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Hugechunk datastructures dump generated by dump_hugechunks_summary_guts in src/c/heapcleaner/heap-debug-stuff.c\n"			);
...
void   dump_hugechunks_summary   (Task* task, char* caller) {
    ...
    dump_hugechunks_summary_guts   (fd, task, caller);
...
// (In practice this means all the compiledfiles in ram.)
//
static void   dump_hugechunks_contents_guts  (FILE* fd, Task* task, char* caller) {
    //        =============================
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Hugechunk datastructures contents dump generated by dump_hugechunks_contents_guts in src/c/heapcleaner/heap-debug-stuff.c\n"		);
    fprintf(fd,"Most of this stuff is defined in one of:\n"												);
    fprintf(fd,"\n"																	);
    fprintf(fd,"    src/c/h/runtime-base.h\n"														);
    fprintf(fd,"    src/c/h/heap.h\n"															);
...
//
static void   dump_ramlog_guts   (FILE*fd, Task* task, char* caller) {
    //        ================
    //
    fprintf(fd,"Ramlog dump generated by dump_ramlog_guts() in src/c/heapcleaner/heap-debug-stuff.c\n");
...
void   dump_ramlog   (Task* task, char* caller) {
    ...
    dump_ramlog_guts (fd, task, caller);
...
void   dump_all   (Task* task, char* caller) {
    ...
    dump_ramlog_guts			(fd, task, caller);
    dump_hugechunks_summary_guts	(fd, task, caller);
    dump_hugechunks_contents_guts	(fd, task, caller);
