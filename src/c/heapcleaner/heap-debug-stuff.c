...
//
void   do_dump_task   (FILE* fd, Task* task, char* caller) {
    // ============
    //
    fprintf(fd,"Task dump generated by do_dump_task in src/c/heapcleaner/heap-debug-stuff.c\n"	);
    fprintf(fd,"\n"										);
...
//
void   do_dump_gen0   (FILE* fd, Task* task, char* caller) {
    // ============
    //
    fprintf(fd,"Generation-0 dump generated by do_dump_gen0 in src/c/heapcleaner/heap-debug-stuff.c\n"	);
    fprintf(fd,"\n"											);
...
void   dump_gen0   (Task* task, char* caller) {
    ...
    do_dump_gen0( fd, task, caller );

    close_heapdump_logfile( fd, filename );
    log_if("dump_gen0: Dump to '%s' now complete.", filename);
}

void   do_dump_gen0s   (FILE* fd, Task* task, char* caller) {
    // =============
    //
...
void   dump_gen0s   (Task* task, char* caller) {
    ...
    //
    char filename[ 1024 ];
    FILE* fd = open_heapdump_logfile( filename, 1024, "gen0s" );

    log_if("dump_gen0s: Starting dump to '%s'", filename);

    do_dump_gen0s( fd, task, caller );
...
// to distinguish live from dead data.
//
void   do_dump_gens   (FILE* fd, Task* task, char* caller) {
    // ============
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Generation-1 dump generated by do_dump_gens in src/c/heapcleaner/heap-debug-stuff.c\n"							);
    fprintf(fd,"\n"																	);
...
//
void   do_dump_hugechunk_stuff   (FILE* fd, Task* task, char* caller) {
    // =======================
    //
    Heap*     heap =  task->heap;

    fprintf(fd,"Hugechunk datastructures dump generated by do_dump_hugechunk_stuff in src/c/heapcleaner/heap-debug-stuff.c\n"				);
...
void   dump_hugechunk_stuff   (Task* task, char* caller) {
    ...
    //
    char filename[ 1024 ];
    FILE* fd = open_heapdump_logfile( filename, 1024, "huge" );

    log_if("dump_hugechunk_stuff: Starting dump to '%s'", filename);

    do_dump_hugechunk_stuff   (fd, task, caller);

    close_heapdump_logfile( fd, filename );
    log_if("dump_hugechunk_stuff: Dump to '%s' now complete.", filename);
}

//
void   do_dump_ramlog( FILE*fd, Task* task, char* caller) {
    //
    fprintf(fd,"Ramlog dump generated by do_dump_ramlog() in src/c/heapcleaner/heap-debug-stuff.c\n");

    fprintf(fd,"\nLast calls made from Mythryl to C, in time order (with task->pthread->id):\n\n");
...
void   dump_ramlog   (Task* task, char* caller) {
    ...
    char filename[ 1024 ];
    FILE* fd = open_heapdump_logfile( filename, 1024, "ramlog" );

    do_dump_ramlog (fd, task, caller);
...
void   dump_whatever   (Task* task, char* caller) {
    ...
    char filename[ 1024 ];
    FILE* fd = open_heapdump_logfile( filename, 1024, "whatever" );
...
void   dump_all   (Task* task, char* caller) {
    ...
    //
    char filename[ 1024 ];
    FILE* fd = open_heapdump_logfile( filename, 1024, "all" );
...
//
void   dump_all_but_huge   (Task* task, char* caller) {
    // =================
    //
    char filename[ 1024 ];
    FILE* fd = open_heapdump_logfile( filename, 1024, "allbuthuge" );

    log_if("dump_all_but_huge: Starting dump to '%s'", filename);

    fprintf(fd,"Dump generated by dump_all_but_huge() in src/c/heapcleaner/heap-debug-stuff.c\n");
    ...
    close_heapdump_logfile( fd, filename );
    log_if("dump_all_but_huge: Dump to '%s' now complete.", filename);
}
