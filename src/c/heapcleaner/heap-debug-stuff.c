...
//
static void   dump_ramlog__guts   (FILE*fd, char* caller, int n) {
    //        =================
    //
	...
	} while (e != ramlog_next_entry_to_write  &&  ((RAMLOG_ENTRIES - 1)-i < n));
    }
}
...
void   dump_ramlog   (Task* task, char* caller) {
    ...
    dump_ramlog__guts (fd, caller, RAMLOG_ENTRIES);
...
//
void   debug_ramlog   (int n) {
    // ============
    //

    FILE* fd     = stderr;
    char* caller = "print_ramlog";
    if (n <= 0             ) n = 10;
    if (n >  RAMLOG_ENTRIES) n = RAMLOG_ENTRIES;
    dump_ramlog__guts (fd, caller, n);
}
