...
#include "system-signals--autogenerated.h"
...
Val   list_signals__may_heapclean   (Task* task, Roots* extra_roots)   {				// Called from src/c/lib/signal/listsignals.c
    //===========================
    //
    return dump_table_as_system_constants_list__may_heapclean (task, &signal_sysconsts_table__local, extra_roots);		// See src/c/heapcleaner/make-strings-and-vectors-etc.c
}
...
void   set_signal_mask   (Task* task, Val arg)   {							// We are called (only) by   src/c/lib/signal/setsigmask.c
	...
	if (LIST_IS_NULL(signal_list)) {
	    ...
	    for (i = 0;  i < NUM_SYSTEM_SIGS;  i++) {
	        //											// signal_sysconsts_table_guts__local	is from   src/c/o/posix-signal-table--autogenerated.c
		ADD_SIGNAL_TO_SET( mask, signal_sysconsts_table_guts__local[i].id );			// ADD_SIGNAL_TO_SET			is from   src/c/h/system-dependent-signal-get-set-etc.h
		...
		int	sig = TAGGED_INT_TO_C_INT(car);
...
Val   get_signal_mask2__may_heapclean   (Task* task, Val arg, Roots* extra_roots)   {			// Called from src/c/lib/signal/getsigmask.c
    //==============================
    // 
    // 'arg' is unused.
    // 
  ...
  //									    ENTER_MYTHRYL_CALLABLE_C_FN(__func__);
	...
	for (i = 0, n = 0;  i < NUM_SYSTEM_SIGS;  i++) {
	    //
	    if (SIGNAL_IS_IN_SET(mask, signal_sysconsts_table_guts__local[i].id))   n++;
	}
	//
	...
	for (i = 0, signal_list = LIST_NIL;   i < NUM_SYSTEM_SIGS;   i++) {
	    ...
	    if (SIGNAL_IS_IN_SET(mask, signal_sysconsts_table_guts__local[i].id)) {
	        //
//		Val name =  make_ascii_string_from_c_string__may_heapclean (task, signal_sysconsts_table_guts__local[i].name, &roots1 );

//		Val sig  =  make_two_slot_record( task, TAGGED_INT_FROM_C_INT(signal_sysconsts_table_guts__local[i].id), name);
//		signal_list = LIST_CONS(task, sig, signal_list);

		signal_list = LIST_CONS(task, TAGGED_INT_FROM_C_INT(signal_sysconsts_table_guts__local[i].id), signal_list);
	    }
	}
    }
...
void   choose_signal   (Hostthread* hostthread)   {			// We are called (only) from   src/c/main/run-mythryl-code-and-runtime-eventloop.c
	...
	//
	if (i == MAX_POSIX_SIGNALS)					// #define MAX_POSIX_SIGNALS  32		in (codemade file:)	src/c/o/system-signals--autogenerated.h
	    i =  MIN_SYSTEM_SIG;					// #define MIN_SYSTEM_SIG      1		in (codemade file:)	src/c/o/system-signals--autogenerated.h
