 ...
 *
 * EAX - temp1 (see the code generator, x86/x86.pkg)
 * EBX - misc0
 * ECX - misc1
 * EDX - misc2
 * ESI - standard fate (lib7_fate, see runtime-state.h)
 * EBP - standard argument (lib7_argument)
 * EDI - free space pointer (lib7_heap_cursor)
 * ESP - stack pointer
 * EIP - program counter (lib7_program_counter)
 */   

/* Registers (see x86/x86.pkg): */
#define temp		EAX
#define misc0		EBX
#define misc1		ECX
#define misc2		EDX
...
#define lib7_state_offset 176
#define lib7state_ptr	REGOFF(lib7_state_offset, ESP)
...
request_w:		/* place to put the request code */
	D_LONG 0
	GLOBL CSYM(LIB7_X86Frame)
LABEL(CSYM(LIB7_X86Frame)) /* ptr to the ml frame (gives C access to limitptr) */
	D_LONG 0		
...
LIB7_CODE_HDR(sigh_return_a)
	MOV_L(CONST(LIB7_void),stdlink)
	MOV_L(CONST(LIB7_void),stdclos)
	MOV_L(CONST(LIB7_void),pc)
 ...
 * standard two-argument function, thus the closure is in lib7_fate.
 */
...
ENTRY(set_request)
	/* temp holds lib7state_ptr, valid request in request_w  */
	/* Save registers */
	MOV_L(lib7state_ptr, temp)
	MOV_L(allocptr, REGOFF(AllocPtrOfflib7_state,temp))
	MOV_L(stdarg, REGOFF(StdArgOfflib7_state,temp))
	MOV_L(stdcont, REGOFF(StdContOfflib7_state,temp))

#define	temp2 allocptr
	/* note that we have left Lib7 code */
	MOV_L(REGOFF(VProcOfflib7_state,temp), temp2)
	MOV_L(CONST(0), REGOFF(InLib7OffVSP,temp2))

	MOV_L(misc0, REGOFF(Misc0Offlib7_state,temp))
	MOV_L(misc1, REGOFF(Misc1Offlib7_state,temp))
	MOV_L(misc2, REGOFF(Misc2Offlib7_state,temp))

	/* Save vregs before the stack frame is popped. */
	MOVE(limitptr,temp2, REGOFF(LimitPtrOfflib7_state,temp))
	MOVE(exncont, temp2, REGOFF(ExnPtrOfflib7_state,temp)) 
	MOVE(stdclos, temp2, REGOFF(StdClosOfflib7_state,temp))
	MOVE(stdlink, temp2, REGOFF(LinkRegOfflib7_state,temp))
	MOVE(pc, temp2, REGOFF(PCOfflib7_state,temp))
	MOVE(storeptr,temp2, REGOFF(StorePtrOfflib7_state,temp))
	MOVE(current_thread_ptr,  temp2, REGOFF(CurrentThreadPtrOfflib7_state,temp))
#undef	temp2	
	
	/* return val of function is request code */
	MOV_L(request_w,creturn)

	/* Pop the stack frame and return to run_lib7(). */
#if defined(OPSYS_DARWIN)
	...
	ALIGNTEXT4
ENTRY(restoreregs)
	MOV_L(REGOFF(4,ESP), temp)	/* Get argument (Lib7state ptr). */
	CALLEE_SAVE
#if defined(OPSYS_DARWIN)
...
#define temp2	EBX
      /* Initialize the Lib7 stack frame. */
	MOVE(REGOFF(ExnPtrOfflib7_state, temp),  temp2, exncont)
	MOVE(REGOFF(LimitPtrOfflib7_state, temp), temp2, limitptr)
	MOVE(REGOFF(StorePtrOfflib7_state, temp), temp2, storeptr)
	MOVE(REGOFF(CurrentThreadPtrOfflib7_state, temp),   temp2, current_thread_ptr)
	LEA_L(CSYM(saveregs), temp2)
	MOV_L(temp2,start_gc)
	MOV_L(temp, lib7state_ptr)

	/* vregs */
	MOVE	(REGOFF(LinkRegOfflib7_state,temp),  temp2, stdlink)
	MOVE	(REGOFF(StdClosOfflib7_state,temp),  temp2, stdclos)

	/* PC */
	MOVE    (REGOFF(PCOfflib7_state,temp), temp2, pc)
#undef	temp2

	/* Load Lib7 registers. */
	MOV_L(REGOFF(AllocPtrOfflib7_state,temp), allocptr)
	MOV_L(REGOFF(StdContOfflib7_state,temp), stdcont)
	MOV_L(REGOFF(StdArgOfflib7_state,temp), stdarg)
	MOV_L(REGOFF(Misc0Offlib7_state,temp), misc0)
	MOV_L(REGOFF(Misc1Offlib7_state,temp), misc1)
	MOV_L(REGOFF(Misc2Offlib7_state,temp), misc2)

	MOV_L(ESP,CSYM(LIB7_X86Frame))	/* frame ptr for signal handler. */

	PUSH_L(misc2)			/* free up a register   */
	PUSH_L(temp)			/* save lib7_state temporarily */

#define	tmpreg	misc2

	/* note that we are entering Lib7 */
	MOV_L(REGOFF(VProcOfflib7_state,temp),temp)  /* temp is now vsp */
#define vsp	temp
	MOV_L(CONST(1),REGOFF(InLib7OffVSP,vsp))
...
restore_and_jmp_lib7:
	POP_L(temp)			/* restore temp to lib7_state */
	POP_L(misc2)
	
jmp_lib7:
	CMP_L(limitptr, allocptr)
	JMP(CODEPTR(REGOFF(PCOfflib7_state,temp)))	/* Jump to Lib7 code. */
...
pending:
	...
	JMP(jmp_lib7)			/* Jump to Lib7 code. */
#undef  vsp
...
LIB7_CODE_HDR(array_a)
	...
	CMP_L(CONST(SMALL_CHUNK_SZW),temp)     /* is this a small chunk */
	JGE(3f)
...
#define temp2 misc1
	...
	OR_L(CONST(MAKE_TAG(DTAG_rw_vec_data)),temp1)
...
#define temp1 misc0
	...
	CMP_L(CONST(SMALL_CHUNK_SZW),temp)
	JGE(2f)
	...
	MOV_L(temp1,REGIND(allocptr))	/* store descriptor */
	ADD_L(CONST(4), allocptr)	/* allocptr++ */
	MOV_L(allocptr, temp1)		/* temp1 := data chunk */
	...
	MOV_L(allocptr, stdarg)		/* stdarg := header chunk */
...
LIB7_CODE_HDR(create_b_a)
	...
	CMP_L(CONST(SMALL_CHUNK_SZW),temp) /* small chunk? */
	JMP(2f)
	JGE(2f)				/* XXXXX */
...
#define temp2   misc1	
	...
	CMP_L(CONST(SMALL_CHUNK_SZW),temp1)
	JGE(3f)


	SHL_L(CONST(TAG_SHIFTW),temp1)	/* build descriptor in temp1 */
	OR_L(CONST(MAKE_TAG(DTAG_ro_vec_data)),temp1)
	MOV_L(temp1,REGIND(allocptr))	/* store descriptor */
	ADD_L(CONST(4),allocptr)	/* allocptr++ */
...
2:
	...
	CMP_L(CONST(LIB7_nil),temp1)	/* temp1 = NIL? */
	JNE(2b)
	...
	MOV_L(allocptr, stdarg)		/* result = header chunk */
	ADD_L(CONST(8),allocptr)	/* allocptr += 2 */
...
#  error multiple processors not supported
	...
	MOV_L(CONST(1), REGIND(stdarg))	/* Set the lock to LIB7_false. */
...
#  error multiple processors not supported
#else /* (MAX_PROCS == 1) */
	MOV_L(CONST(3), REGIND(stdarg))		/* Store LIB7_true into lock. */
 ...
 * floating point register stack are claimed (see x86/x86.pkg).
 *
 ...
 * caller-save, so we can use it here (see x86/x86.pkg). */
