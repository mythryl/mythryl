...
LIB7_CODE_HDR(sigh_return_a)
	...
	li	stdlink, LIB7_void
	li	stdclos, LIB7_void
	li	pc, LIB7_void
	b	set_request
...
ENTRY(pollh_resume)
	...
	b	set_request
		 ...
		 /* exception handler for Lib7 functions called from C */
LIB7_CODE_HDR(handle_a)
	...
	b	set_request
		...
		/* fate for Lib7 functions called from C */		
...
set_request:
	lwz	atmp3,LIB7STATE_OFFSET(sp)	/* save the minimal Lib7 state */
	lwz	atmp2,VProcOfflib7_state(atmp3)	/* atmp2 := VProc State ptr */
	li	r0,0
	stw	r0,InLib7OffVSP(atmp2)		/* note that we have left Lib7 */
	stw	allocptr,AllocPtrOfflib7_state(atmp3)
	stw	limitptr,LimitPtrOfflib7_state(atmp3)
	stw	storeptr,StorePtrOfflib7_state(atmp3)
	stw	stdlink,LinkRegOfflib7_state(atmp3)
	stw	pc,PCOfflib7_state(atmp3)
	stw	stdarg,StdArgOfflib7_state(atmp3)
	stw	stdcont,StdContOfflib7_state(atmp3)
	stw	stdclos,StdClosOfflib7_state(atmp3)
	stw	current_thread_ptr,CurrentThreadPtrOfflib7_state(atmp3)
	stw	exncont,ExnPtrOfflib7_state(atmp3)
	stw	miscreg0,Misc0Offlib7_state(atmp3)
	stw	miscreg1,Misc1Offlib7_state(atmp3)
	stw	miscreg2,Misc2Offlib7_state(atmp3)
...
CENTRY(restoreregs)
	...
	and	atmp1,r3,r3			/* atmp1 := Lib7state pointer */

	lwz	allocptr,AllocPtrOfflib7_state(atmp1)
	lwz	limitptr,LimitPtrOfflib7_state(atmp1)
	lwz	storeptr,StorePtrOfflib7_state(atmp1)
	lwz	atmp2,VProcOfflib7_state(atmp1)	/* atmp2 := VProc State ptr */
	li	atmp3,1
	stw	atmp3,InLib7OffVSP(atmp2)         /* we are entering Lib7 code */
	lwz	stdarg,StdArgOfflib7_state(atmp1)
	lwz	stdcont,StdContOfflib7_state(atmp1)
	lwz	stdclos,StdClosOfflib7_state(atmp1)
	lwz	exncont,ExnPtrOfflib7_state(atmp1)
	lwz	miscreg0,Misc0Offlib7_state(atmp1)
	lwz	miscreg1,Misc1Offlib7_state(atmp1)
	lwz	miscreg2,Misc2Offlib7_state(atmp1)
	lwz	stdlink,LinkRegOfflib7_state(atmp1)
	lwz	current_thread_ptr,CurrentThreadPtrOfflib7_state(atmp1)
	lwz	atmp3,PCOfflib7_state(atmp1)
	mtlr	atmp3
...
LIB7_CODE_HDR(array_a)
	...
	cmpi	CR0,atmp2,SMALL_CHUNK_SZW /* is this a small chunk */
	...
	addi	allocptr,allocptr,4	/* points to new chunk */
...
LIB7_CODE_HDR(create_b_a)
	...
	addi	allocptr,allocptr,4	/* allocptr++ */
	addi	atmp3, allocptr, 0	/* atmp3 = data chunk */
	...
	addi	stdarg, allocptr, 0	/* stdarg = header chunk */
...
create_v_a_1:
	...
	addi	stdarg, allocptr, 0	/* result = header chunk */
	addi	allocptr,allocptr,8	/* allocptr += 2 */
	CONTINUE
