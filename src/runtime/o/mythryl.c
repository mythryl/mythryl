...
/* Created 2007-03-12 CrT:  Under LGPL. */ 
 ...
 * which gets invoked by a "#/usr/bin/runtime7" shebang line at
...
close_logfile (void)
{
    fprintf(log_fd,"src/runtime/o/mythryl.c: close_logfile()\n");
    fclose(log_fd);
}
#endif
...
#define SCRIPT_EXIT_BOILERPLATE "\n posix::kill (posix::K_PROC (posix::get_process_id' () ), posix::signal::term );;\n"
...
static void
allocate_script_buffer   ( int size_in_bytes ) {
    ...
    text_for_compiler_buffer
        =
        (char*) malloc( size_in_bytes + strlen( SCRIPT_EXIT_BOILERPLATE ) + 2 );	/* "+2" for a newline and nul. */

    if (!text_for_compiler_buffer) {
        fprintf( stderr, "%s: Unable to allocate %d bytes for script buffer: %s\n", our_name, size_in_bytes, strerror( errno ) );
        exit(1);
    }
...
static void
read_script_into_buffer   ( char* scriptname ) {
...
static int
args_to_skip   (int argc, char** argv) {
...
static Stdin_Stdout_Stderr_Pipes
open_subprocess_pipes   ( void ) {
...
static void
sigchld_handler(   int signal, siginfo_t* info, void* context   ) {
...
/* Handle ^C: */
static void
sigterm_handler(   int signal, siginfo_t* info, void* context   ) {
...
static void
set_signal_handler( int signum, void (*handler)(int signum, siginfo_t* info, void* context) ) {
...
static void
run_subprocess_to_conclusion( Stdin_Stdout_Stderr_Pipes subprocess_pipes ) {
...
static void
run_compiler_subprocess   ( char** argv ) {
