/* card-map.h
 *
 * Card maps for marking chunk updates.
 */
...
typedef struct {		      /* A dirty card map */
    lib7_val_t		*baseAddr;	/* The base address of the mapped region */
...
#define isDIRTY(cm, index, maxGen)	((cm)->map[(index)] <= (maxGen))
 ...
 * required when setting a bit, but this doesn't seem to hold for lib7.  I
...
/* test a card to see if it is marked */
#define isDIRTY(cm, index)	\
    ((cm)->map[(index) >> LOG_BITS_PER_WORD] & (1 << ((index) & (BITS_PER_WORD-1))))
