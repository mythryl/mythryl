/* mythryl-gtk-slave.c -- a simple Gtk subprocess slave for Mythryl.
 */
 ...
 *     display and response loops in mythryl-gtk-slave can
...
die(void)
{
    printf( "FATAL src/runtime/gtk/mythryl-gtk-slave.c: %s  exit(1)ing.\n", text_buf );
...
open_logfile (void)
{
    ...
    sprintf (filename, "mythryl-gtk-slave-%d.log~", getpid());
 ...
 * A hash table would be more conventional, but
...
make_trie_node (void)
{
    ...
    if (!result) {  sprintf( text_buf, "make_trie_node: Couln't allocate node." ); die(); }
...
do_init( int argc, unsigned char** argv )
{
    ...
    if (!gtk_init_check( &main_argc, &main_argv )) {
      ...
      sprintf( text_buf, "do_init: failed to initialize GUI support.");  die();
    } else {
...
/* Above fn generated by write_mythryl_gtk_slave_c_plain_fun. */
...
/* Above fn generated by write_gtk_slave_c_set_callback_fun. */
...
read_one_line (void)
{
    ...
    while ((c = getchar()) != '\n') {
        ...
        if (c == EOF)   { sprintf( text_buf, "read_one_line: EOF on input." ); die(); }

        if (line_end - line_buf >= MAX_LINE_LENGTH) {
	    sprintf( text_buf, "read_one_line: Line greater than MAX_LINE_LENGTH." );  die();
        }
...
parse_line (void)
{
    ...
    for (;;) {
        ...
        if (argc >= MAX_ARGC)  { sprintf(text_buf, "parse_line: More than MAX_ARGC words on input line."); die(); }
    }
}
