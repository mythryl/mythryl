...
* History:
    Based loosely on libg++-1.2X malloc. (It retains some of the overall 
       package of old version,  but most details differ.)
    ...
    The allocation strategy has several phases:
      ...
      2. Look for a dirty chunk of exact size in the bin associated
         ...
         2a If there are dirty chunks, but none of the right size,
             ...
             random walks through memory that degrade
...
size_t malloc_usable_size(void* mem)
{
  ...
  else
  {
    ...
    size_t size = p->size & ~(INUSE);
    /* report zero if not in use or detectably corrupt */
    if (p->size == size || size != *((size_t*)((char*)(p) + size - SIZE_SZ)))
      return 0;
    else
      return size - MALLOC_MIN_OVERHEAD;
  }
}
