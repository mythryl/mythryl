...
// Qqq binding.  The Mythryl side
// is implemented by
//
//     src/opt/qqq/src/qqq-client-driver-for-library-in-main-process.pkg
//
// We get compiled by:
//    src/opt/qqq/c/in-main/Makefile.in
...
static void   moan_and_die   (void)   {
    //        ============
    //
    printf( "FATAL src/c/lib/qqq/mythryl-qqq-library-in-main-process.c: %s  exit(1)ing.\n", text_buf );		fflush(stdout);
    exit(1);
}
...
//
// Consequently when Qqq issues a
...
printf("get_next_queued_callback called...  --libmythryl-qqq.c\n");
    if (callback_queue_is_empty()) {
...
void dummy_qqq_call_to__queue_up_void_callback() { queue_up_void_callback(1); }		// Can delete this as soon as we have some real calls -- this is just to quiet gcc.
...
static void   queue_up_bool_callback   (int callback, int bool_value)   {
    ...
    queue_up_callback( e );
										...
										void dummy_qqq_call_to__queue_up_bool_callback() { queue_up_bool_callback(1,TRUE); }		// Can delete this as soon as we have some real calls -- this is just to quiet gcc.
...
static void   queue_up_float_callback   (int callback,  double float_value)   {
    ...
    queue_up_callback( e );
										...
										void dummy_qqq_call_to__queue_up_float_callback() { queue_up_float_callback(1,1.0); }		// Can delete this as soon as we have some real calls -- this is just to quiet gcc.
...
static int   find_free_callback_id   ()   {
    ...
    return next_callback_id++;
										...
										void dummy_qqq_call_to__find_free_callback_id() { find_free_callback_id(); }		// Can delete this as soon as we have some real calls -- this is just to quiet gcc.
...
// qqq-client.api        type:   (None -- not exported to qqq-client.api level.)
// qqq-client-driver.api type:   Void -> Bool
//
...
// qqq-client.api        type:   (None -- not exported to qqq-client.api level.)
// qqq-client-driver.api type:   Void -> Int
//
...
// qqq-client-driver.api type:   Void -> Int
//
Val   _lib7_Qqq_get_queued_void_callback   (Task* task,  Val arg)   {
    //==================================
    //
...
// qqq-client.api        type:   (None -- not exported to qqq-client.api level.)
// qqq-client-driver.api type:   Void -> (Int, Bool)
//
Val   _lib7_Qqq_get_queued_bool_callback   (Task *task,  Val arg)   {
    //==================================
    //
    Callback_Queue_Entry e = get_next_queued_callback ();
    //
...
// qqq-client.api        type:   (None -- not exported to qqq-client.api level.)
// qqq-client-driver.api type:   Void -> (Int, Float)
//
Val   _lib7_Qqq_get_queued_float_callback   (Task* task, Val arg)  {
    //===================================
    //
    Callback_Queue_Entry e = get_next_queued_callback ();
    //
...
// qqq-client.api        type:   (None -- not exported to qqq-client.api level.)
// qqq-client-driver.api type:   Void -> (Int,     Int)
//                                 callback x      y
//
...
//     fn-type:
// drive the code-build path
//     mlb::BUILD_A ("callback-fn", build_callback_function)			# In src/opt/qqq/sh/make-qqq-glue
//  ->  build_callback_function							# In src/opt/qqq/sh/make-qqq-glue
//  ->  build_set_callback_fn_for_'libmythryl_xxx_c'				# In src/opt/qqq/sh/make-qqq-glue
 ...
 *
 * qqq-client.api        type:    Session -> Void
 * qqq-client-driver.api type:   (Session) -> Void
 */
...
/* Above fn built by src/lib/make-library-glue/make-library-glue.pkg:  write_libmythryl_xxx_c_plain_fun  per  src/opt/qqq/etc/qqq-construction.plan. */


/* do__negate_int
 *
 * qqq-client.api        type:   (Session, Int) -> Int
 * qqq-client-driver.api type:   (Session, Int) -> Int
 */
...
/* do__negate_float
 *
 * qqq-client.api        type:   (Session, Float) -> Float
 * qqq-client-driver.api type:   (Session, Float) -> Float
 */
...
/* do__negate_boolean
 *
 * qqq-client.api        type:   (Session, Bool) -> Bool
 * qqq-client-driver.api type:   (Session, Bool) -> Bool
 */
...
/////////////// old libmythryl-qqq.c contents follow //////////////////////////////////
...
//
Mythryl_Callable_C_Library	    Libmythryl_Qqq = {						// Mythryl_Callable_C_Library		def in    src/c/h/mythryl-callable-c-libraries.h
    //                              ==================
    CLIB_NAME,
    CLIB_VERSION,
    CLIB_DATE,
