// mythryl-qqq-library-in-c-subprocess.c -- a simple Qqq subprocess server for Mythryl.
...
static void   moan_and_die   (void)   {
    //        ============
    //
    printf( "src/c/qqq/mythryl-qqq-library-in-c-subprocess.c:  Fatal error:  %s  exit(1)ing.\n", text_buf );
...
open_logfile (void)
{
    ...
    sprintf (filename, "mythryl-qqq-library-in-c-subprocess-%d.log~", getpid());
...
string_arg( int iargc, unsigned char** argv, int ii)
{
    ...
    return (char*) argv[ ii ];
										...
										void dummy_qqq_call_to__string_arg() { unsigned char*argv[10]; string_arg(1,argv,2); }	// Can delete this as soon as we have some real calls -- this is just to quiet gcc.
...
//                                                              # from src/opt/qqq/sh/make-qqq-glue
// 
// Paragraphs like
//     build-a: callback-fn
//     fn-name:
...
//       ->  build_set_callback_fn_for_'mythryl_xxx_library_in_c_subprocess_c'	# In src/opt/qqq/sh/make-qqq-glue
...
/* Above fn built by src/lib/make-library-glue/make-library-glue.pkg:  build_plain_fun_for_'mythryl_xxx_library_in_c_subprocess_c'  per  src/opt/qqq/etc/qqq-construction.plan. */
