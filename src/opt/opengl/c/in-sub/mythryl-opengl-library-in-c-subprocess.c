// mythryl-opengl-library-in-c-subprocess.c -- a simple Opengl subprocess server for Mythryl.
...
void mythryl_opengl_server_dummy( void ) {				// This just a test to see if the appropriate libraries are linking; I don't intend to actually call this fn. It is non-static only to keep gcc from muttering about unused code.
    //
    int running = GL_TRUE;
...
static void   moan_and_die   (void)   {
    //        ============
    //
    printf( "src/c/opengl/mythryl-opengl-library-in-c-subprocess.c:  Fatal error:  %s  exit(1)ing.\n", text_buf );
...
open_logfile (void)
{
    ...
    sprintf (filename, "mythryl-opengl-library-in-c-subprocess-%d.log~", getpid());
...
{
printf("do_init/top -- src/opt/opengl/c/in-sub/mythryl-opengl-library-in-c-subprocess.c\n");fflush(stdout);
    if (!glfwInit()) {
	//
...
// printf("do_init/AAA: calling glewInit -- src/opt/opengl/c/in-sub/mythryl-opengl-library-in-c-subprocess.c\n");fflush(stdout);
...
//                                                              # from src/opt/opengl/sh/make-opengl-glue
// 
// Paragraphs like
//     build-a: callback-fn
//     fn-name:
...
//       ->  build_set_callback_fn_for_'mythryl_xxx_library_in_c_subprocess_c'	# In src/opt/opengl/sh/make-opengl-glue
...
/* Above fn built by src/lib/make-library-glue/make-library-glue.pkg:  build_set_callback_fn_for_'mythryl_xxx_library_in_c_subprocess_c'  per  src/opt/opengl/etc/opengl-construction.plan.*/
...
/* Above fn built by src/lib/make-library-glue/make-library-glue.pkg:  build_plain_fun_for_'mythryl_xxx_library_in_c_subprocess_c'  per  src/opt/opengl/etc/opengl-construction.plan. */
